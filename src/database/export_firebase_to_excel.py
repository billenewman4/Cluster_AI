...
def export_firestore_to_excel(collection_name, output_file=None):
    ...
    try:
        ...
        print("Error: Firebase MCP client not available. Using manual data export...")
        return export_manual_data(collection_name, output_file)
    except Exception as e:
        print(f"Error exporting data: {str(e)}")
        return None

def export_manual_data(collection_name, output_file=None):
    """
    Manual export using sample data (fallback method)
    """
    # Sample data from the Firebase query result
    sample_data = [
        {
            'id': 'item_000000',
            'product_code': '100101',
            'product_family': 'Beef Chuck Scotty Tender No-Roll',
            'product_description': 'BF CHUCK SCOTCH TENDER NR AME 116B-1 USDA NO ROLL',
            'brand': 'AMERICAN',
            'grade': 'No-Roll',
            'category_description': 'Beef Chuck',
            'subprimal': 'Scotty Tender',
            'bone_in': False,
            'confidence': 0.9,
            'needs_review': True,
    ...
    print(f"Manual export completed: {output_path}")
    return output_path
